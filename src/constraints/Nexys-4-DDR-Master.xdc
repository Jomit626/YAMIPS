...
set _xlnx_shared_i0 [get_pins -filter { NAME =~  "*regster*" && NAME =~  "*C*" && NAME !~  "*CE*" } -of_objects [get_cells -hierarchical -filter { NAME =~  "*regfile_inst_0*" }]]
set_false_path -from $_xlnx_shared_i0 -to [get_pins -filter { NAME =~  "*pc_reg*" || NAME =~  "*pc_dup_reg*" && NAME =~  "*D*" } -of_objects [get_cells -hierarchical -filter { NAME =~  "*if_unit*" }]]
